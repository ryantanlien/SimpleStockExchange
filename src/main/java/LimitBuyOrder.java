...
public class LimitBuyOrder implements BuyOrder {
    ...
    public LimitBuyOrder(String stockTicker, double askingPrice, int stockNumber, int orderId) {
    ...
    public double getPriceBought() {
        ...
        double priceBought;
        if (totalVolume != 0) {
            priceBought = totalPrice/totalVolume;
        } else {
            priceBought = 0;
        }
        return priceBought;
    ...
    public String toString() {
        String string = this.getStockTicker() + " LMT BUY $" + getAskingPrice() + " " + this.getFulfilledNumber() + "/"
